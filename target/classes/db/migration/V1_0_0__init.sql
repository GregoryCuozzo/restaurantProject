...
alter table restaurants add constraint FK_restoUser foreign key (XIDadmin) REFERENCES users(id_user);
alter table restaurants add constraint FK_restoVille foreign key (XIDville) REFERENCES villes(id_ville);

alter table villes add constraint FK_ville foreign key (XIDpays) REFERENCES pays(id_pays);
